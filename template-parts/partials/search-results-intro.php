...
if ($show_search_form) {
    ...
    <button
        type="submit"
        class="btn inline-flex h-[40px] items-center justify-center gap-2 rounded-[6px] bg-[#08284B] px-4 text-[14px] font-medium leading-[24px] text-white"
    >
...
<div class="flex flex-col gap-8">
    <?php if ($show_search_form) { ?>
        <?php if ($render_form_tag) { ?>
            <form action="<?php echo esc_url($search_base_url); ?>" method="get" class="flex w-full max-w-[384px] flex-col gap-3 sm:flex-row">
