...
/**
 * Seed Refer to the St Patrick's at Home Service (page 235) to match Figma frame 2888:4337.
 *
 * Run: wp eval-file wp-content/themes/matrix-starter/scripts/seed-refer-to-the-st-patricks-at-home-service.php
 */

$post_id = (int) (get_page_by_path('healthcare-professionals/refer-to-the-st-patricks-at-home-service')?->ID ?? 0);

if ($post_id === 0) {
    if (class_exists('WP_CLI')) {
        WP_CLI::error('Could not find page at healthcare-professionals/refer-to-the-st-patricks-at-home-service.');
    }
...
$figma = [
    'video' => 'https://www.figma.com/api/mcp/asset/f1a00937-e189-492d-8527-cbbd236356fd',
    'testimonials_bg' => 'https://www.figma.com/api/mcp/asset/c7c208ee-eeb0-4bfa-a0ba-3a26fff12083',
];

$video_poster_id = matrix_seed_resolve_image($figma['video'], 'refer-at-home-video-2888-4337', 'Refer to at home service video');
$testimonials_bg_id = matrix_seed_resolve_image($figma['testimonials_bg'], 'refer-at-home-testimonials-2888-4337', 'Refer to at home service testimonials background');
...
$flexi_rows = [
    [
        ...
        'current_crumb_label' => 'Refer to At Home Service',
        'heading_tag' => 'h1',
        'heading' => 'Refer to At Home Service',
...
if (class_exists('WP_CLI')) {
    if ($saved_count === count($flexi_rows)) {
        WP_CLI::success(sprintf(
            'Seeded Refer to the St Patrick\'s at Home Service page (%d) with %d flexi blocks.',
            $post_id,
            $saved_count
        ));
    } else {
